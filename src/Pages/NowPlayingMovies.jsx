 ...
 * NowPlaying function
 *  
 * This function renders the movies being played in cinemas now by calling the now playing api and then proceeding to render the movies using the MoviesListings component
 * 
 * @name NowPlaying
 * @function
 * @param {Int16Array} page the current page to be rendered
 * @param {String} language the language by which to retrieve the current movies
 * @param {String} region the region from which to retrieve the current movies
 ...
 * @param {Dispatch} append_movies the dispatch action to add an array to the current movie state
 ...
 */
function NowPlaying({page, language, region, genre, adult, movies, render, append_movies, set_page, clear_movies, error}) {
    ...
    useEffect(() => {
        ...
        async function GetMovies() {
            SetMaxPage(await Api_NowPlaying(append_movies, page, language, region, genre, adult, error));
        }
