 ...
 * TopRated function
 *  
 * This function renders the top rated movies by calling the top rated api and then proceeding to render the movies using the MoviesListings component
 * 
 * @name TopRated
 * @function
 * @param {Int16Array} page the current page to be rendered
 * @param {String} language the language by which to retrieve the top rated movies
 * @param {String} region the region from which to retrieve the top rated movies
 ...
 * @param {Dispatch} append_movies the dispatch action to add an array to the top rated movie state
 ...
 */
function TopRated({page, language, region, genre, adult, movies, render, append_movies, set_page, clear_movies, error}) {
    ...
    useEffect(() => {
        async function GetMovies() {
            SetMaxPage(await Api_TopRated(append_movies, page, language, region, genre, adult, error));
        }
